...
def load_binance_withdrawals_file(file):
    try:
        return pd.read_csv(file, index_col=False)
    except Exception as e:
        print(f'[-] Error: {e}')

def convert_binance_withdrawals_to_cointracker(df):
    ...
    df['Sent Quantity'] = df['Amount']
    df['Sent Currency'] = df['Coin']

    df['Fee Currency'] = df['Coin']
    df['Fee Amount'] = df['TransactionFee']

    # zero these out for withdrawals
    df['Received Currency'] = ''
    df['Received Quantity'] = ''
...
def main(args):
    ...
    # Load the CSV file
    df = load_binance_withdrawals_file(args.file)

    # Convert the transactions
    cointracker_dataframe = convert_binance_withdrawals_to_cointracker(df)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output', type=str, default='./cointracker_binance_withdrawals.csv', metavar='',help="Location to output the Cointracker compatible CSV file")
